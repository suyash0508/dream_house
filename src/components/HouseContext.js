...
const HouseContextProvider = ({ children }) => {
  ...
  const [country, setCountry] = useState('Location (any)');   // selected one
  const [countries, setCountries] = useState([]);             // All
  const [property, setProperty] = useState('Property type (any)');  //selected one
  ...
  useEffect(() => {
    ...
    // remove duplicates
    const uniqueCountries = ['Location (any)', ...new Set(allCountries)];  //only kanpur and varanasi
  ...
  useEffect(() => {
    ...
    // remove duplicates
    const uniqueProperties = ['Property type (any)', ...new Set(allProperties)];  // only apartment and house
